...
def change_test_mtl_index_save(model, data_loader, out_root, output_prob=False,
...
#       assert palette.shape[0] == len(model.module.CLASSES)
    ...
    for index,data in enumerate(data_loader):
        ...
        print(ori_filename)
        # if 'beijing' in ori_filename:
        #     continue

        out_fn1 = osp.join(out_dir0, str(index) + "_"+fn_pre+'.png')
        if palette is not None:
            out_fn2 = osp.join(out_dir0_color, str(index) + "_"+fn_pre+'.png')
