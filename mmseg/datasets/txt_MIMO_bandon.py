...
class TxtMIMODatasetForBANDON(Dataset):
    ...
    def load_annotations(self, data_root, txt_fn):
        ...
        for i, line in enumerate(list_read):
            ...
            if data_root is not None:
                ...
                # raise Exception(data_root,line_split)
            # assume that the size of image is all the same
            if not self.has_mask:
                img_info = dict(filename=[name for name in line_split])
            else:
                img_info = dict(filename=[name for name in line_split[:2]])

                seg_build_path = [name for name in line_split[3:5]]

                flow_build_path = [name for name in line_split[5:7]]

                flow_cd_path = line_split[-1]

                img_info['ann'] = dict(seg_map=line_split[2], seg_build=seg_build_path,
                                       flow_build=flow_build_path, flow_cd=flow_cd_path)
    ...
    def evaluate_dist(self, pred_dir, metric='mIoU', logger=None, **kwargs):
        ...
        if metric == 'mIoU':
            ...
            param_eval[
                'gt_ins_area_min'] = 100  # minimum area of valid instance for gt (the instance of gt whose area is lower than 'gt_ins_area_min' will be removed when evaluating)
            param_eval['ins_IoU_thr'] = 0.1

            gt_ins_area_list = list(
                set([0, 100, 200, 400, 900, 1600, 2500, 3600, 4900, 6400, 8100, 10000, param_eval['gt_ins_area_min']]))
            gt_ins_area_list.sort()
            param_eval['gt_ins_area_list'] = gt_ins_area_list  # list of thresholds to compute recalls

            eval_out_root = pp_dir + '_GtInsMin' + str(param_eval['gt_ins_area_min']) + '_insIoUthr' + str(
                param_eval['ins_IoU_thr']) + '_nonStrict'
