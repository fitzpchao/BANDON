...
class EncoderDecoder(BaseSegmentor):
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg, seg_weight_map=None):
    ...
    def _auxiliary_head_forward_train(self, x, img_metas, gt_semantic_seg, seg_weight_map=None):
    ...
    def slide_inference(self, img, img_meta, rescale):
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                crop_seg_logit = self.encode_decode(crop_img, img_meta) * crop_weight_mat
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        seg_pred = self.inference(imgs[0], img_metas[0], rescale)
        for i in range(1, len(imgs)):
            cur_seg_pred = self.inference(imgs[i], img_metas[i], rescale)
            seg_pred += cur_seg_pred
        seg_pred /= len(imgs)
        seg_pred = seg_pred.cpu().detach().numpy()
        # unravel batch dim
        seg_pred = list(seg_pred)
