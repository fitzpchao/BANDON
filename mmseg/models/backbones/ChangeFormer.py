...
#Difference module
def conv_diff(in_channels, out_channels,conv_cfg,norm_cfg):
...
def make_prediction(in_channels, out_channels,conv_cfg,norm_cfg):
    return nn.Sequential(
        ConvModule(
            in_channels,
            out_channels,
...
class EncoderTransformer_v3(nn.Module):
    def __init__(self, img_size=256, patch_size=3, in_chans=3,  embed_dims=[32, 64, 128, 256],
                 num_heads=[2, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 3, 6, 18], sr_ratios=[8, 4, 2, 1],pretrained=None):
        super().__init__()
...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
                    ...
                    decoder_softmax = False, feature_strides=[2, 4, 8, 16],conv_cfg=None,norm_cfg=None):
        ...
        self.diff_c4   = conv_diff(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.diff_c3   = conv_diff(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.diff_c2   = conv_diff(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.diff_c1   = conv_diff(in_channels=2*self.embedding_dim, out_channels=self.embedding_dim,conv_cfg=conv_cfg,norm_cfg=norm_cfg)

        #taking outputs from middle of the encoder
        self.make_pred_c4 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.make_pred_c3 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.make_pred_c2 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc,conv_cfg=conv_cfg,norm_cfg=norm_cfg)
        self.make_pred_c1 = make_prediction(in_channels=self.embedding_dim, out_channels=self.output_nc,conv_cfg=conv_cfg,norm_cfg=norm_cfg)

        #Final linear fusion layer
        self.linear_fuse = nn.Sequential(
            ConvModule(
                self.embedding_dim*len(in_channels),
                self.embedding_dim,
                kernel_size=1,
                conv_cfg=conv_cfg,
                norm_cfg=norm_cfg
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c4_1 = self.linear_c4(c4_1).permute(0,2,1).reshape(n, -1, c4_1.shape[2], c4_1.shape[3])
        _c4_2 = self.linear_c4(c4_2).permute(0,2,1).reshape(n, -1, c4_2.shape[2], c4_2.shape[3])
        _c4   = self.diff_c4(torch.cat((_c4_1, _c4_2), dim=1).contiguous())
        p_c4  = self.make_pred_c4(_c4.contiguous())
        outputs.append(p_c4)
        ...
        _c3_1 = self.linear_c3(c3_1).permute(0,2,1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        _c3_2 = self.linear_c3(c3_2).permute(0,2,1).reshape(n, -1, c3_2.shape[2], c3_2.shape[3])
        _c3   = self.diff_c3(torch.cat((_c3_1, _c3_2), dim=1).contiguous()) + F.interpolate(_c4, scale_factor=2, mode="bilinear")
        p_c3  = self.make_pred_c3(_c3.contiguous())
        outputs.append(p_c3)
        ...
        _c2_1 = self.linear_c2(c2_1).permute(0,2,1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        _c2_2 = self.linear_c2(c2_2).permute(0,2,1).reshape(n, -1, c2_2.shape[2], c2_2.shape[3])
        _c2   = self.diff_c2(torch.cat((_c2_1, _c2_2), dim=1).contiguous()) + F.interpolate(_c3, scale_factor=2, mode="bilinear")
        p_c2  = self.make_pred_c2(_c2.contiguous())
        outputs.append(p_c2)
        ...
        _c1_1 = self.linear_c1(c1_1).permute(0,2,1).reshape(n, -1, c1_1.shape[2], c1_1.shape[3])
        _c1_2 = self.linear_c1(c1_2).permute(0,2,1).reshape(n, -1, c1_2.shape[2], c1_2.shape[3])
        _c1   = self.diff_c1(torch.cat((_c1_1, _c1_2), dim=1).contiguous()) + F.interpolate(_c2, scale_factor=2, mode="bilinear")
        p_c1  = self.make_pred_c1(_c1.contiguous())
        outputs.append(p_c1)

        #Linear Fusion of difference image from all scales
        _c = self.linear_fuse(torch.cat((_c4_up, _c3_up, _c2_up, _c1), dim=1).contiguous())
...
class DecoderTransformer_v4(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True,
        ...
        # convolutional Difference Modules
        self.diff_c4 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim, conv_cfg=conv_cfg,
                                 norm_cfg=norm_cfg)
        self.diff_c3 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim, conv_cfg=conv_cfg,
                                 norm_cfg=norm_cfg)
        self.diff_c2 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim, conv_cfg=conv_cfg,
                                 norm_cfg=norm_cfg)
        self.diff_c1 = conv_diff(in_channels=2 * self.embedding_dim, out_channels=self.embedding_dim, conv_cfg=conv_cfg,
                                 norm_cfg=norm_cfg)
    ...
    def forward(self, inputs1, inputs2):
        ...
        _c3_1 = self.linear_c3(c3_1).permute(0, 2, 1).reshape(n, -1, c3_1.shape[2], c3_1.shape[3])
        _c3_2 = self.linear_c3(c3_2).permute(0, 2, 1).reshape(n, -1, c3_2.shape[2], c3_2.shape[3])
        _c3 = self.diff_c3(torch.cat((_c3_1, _c3_2), dim=1).contiguous())
        ...
        _c2_1 = self.linear_c2(c2_1).permute(0, 2, 1).reshape(n, -1, c2_1.shape[2], c2_1.shape[3])
        _c2_2 = self.linear_c2(c2_2).permute(0, 2, 1).reshape(n, -1, c2_2.shape[2], c2_2.shape[3])
        _c2 = self.diff_c2(torch.cat((_c2_1, _c2_2), dim=1).contiguous())
        ...
        _c1_1 = self.linear_c1(c1_1).permute(0, 2, 1).reshape(n, -1, c1_1.shape[2], c1_1.shape[3])
        _c1_2 = self.linear_c1(c1_2).permute(0, 2, 1).reshape(n, -1, c1_2.shape[2], c1_2.shape[3])
        _c1 = self.diff_c1(torch.cat((_c1_1, _c1_2), dim=1).contiguous())
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, img_size=512, decoder_softmax=False, embed_dim=256,pretrained=None,conv_cfg=None,norm_cfg=None):
        ...
        mit = EncoderTransformer_v3(img_size=img_size, patch_size = 3, in_chans=input_nc,  embed_dims=self.embed_dims,
                 ...
                 depths=self.depths, sr_ratios=[8, 4, 2, 1],pretrained=pretrained)
        # self.Tenc_x2._init_weights(pretrained)
        self.Tenc_x2 = mit
        #Transformer Decoder
        self.TDec_x2   = DecoderTransformer_v3(input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=False,
                    ...
                    decoder_softmax = decoder_softmax, feature_strides=[2, 4, 8, 16],conv_cfg=conv_cfg,norm_cfg=norm_cfg)

    def init_weights(self,pretrained=None):
        pass

        # self.Tenc_x2.load_state_dict(torch.load(pretrained), strict=False)
