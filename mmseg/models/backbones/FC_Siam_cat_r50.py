...
class Siam_cat_r50(nn.Module):
    ...
    def __init__(self,
                 ...
                 norm_eval=False,
                 **kwargs):
        super(Siam_cat_r50, self).__init__()
        ...
        if self.use_IN1:
            if deep_stem:
                resnet.stem[0] =  build_conv_layer(
                    conv_cfg,
                    6,
                    stem_channels // 2,
                    kernel_size=3,
                ...
                self.__init_weights__(resnet.stem[0].modules())
                ...
                conv1 = build_conv_layer(
                    ...
                    padding=3,
                    bias=False)
                self.__init_weights__(conv1.modules())
                self.layer0 = nn.Sequential(conv1,
        ...
        self.decoder_4 = decoder_block(in_channels=backbone_channels[3] , out_channels=backbone_channels[2],
                                       norm_cfg=norm_cfg)
        self.decoder_3 = decoder_block(in_channels=backbone_channels[2] * 2, out_channels=backbone_channels[1],
                                       norm_cfg=norm_cfg)
        self.decoder_2 = decoder_block(in_channels=backbone_channels[1] * 2, out_channels=backbone_channels[0],
                                       norm_cfg=norm_cfg)
        self.decoder_1 = decoder_block(in_channels=backbone_channels[0] * 2, out_channels=backbone_channels[0],
                                       norm_cfg=norm_cfg)
    ...
    def __init_weights__(self,modules):
    ...
    def forward(self, x1, x2):
        ...
        features= self._forward_embedding(torch.cat([x1,x2],1))

        F4 = self.decoder_4(features[3])  # H/32
        F4 = F.interpolate(F4, size=features[2].shape[2:], mode="bilinear", align_corners=True)  # H/16

        F3_cat = torch.cat([features[2], F4], dim=1)
        F3 = self.decoder_3(F3_cat)  # H/16
        F3 = F.interpolate(F3, size=features[1].shape[2:], mode="bilinear", align_corners=True)  # H/8

        F2_cat = torch.cat([features[1], F3], dim=1)
        F2 = self.decoder_2(F2_cat)  # H/8
        F2 = F.interpolate(F2, size=features[0].shape[2:], mode="bilinear", align_corners=True)  # H/4

        F1_cat = torch.cat([features[0] - features[0], F2], dim=1)
    ...
    def train(self, mode=True):
        super(Siam_cat_r50, self).train(mode)
        # self._freeze_stages()
